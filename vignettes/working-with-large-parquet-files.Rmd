...
- `genefamilies_stratified_gene_family_uniref.parquet` - sorted by `gene_family_uniref` column
...
- Filter by sorted columns (e.g., `uuid`, `gene_family_uniref`) when querying the remote parquet files
- This creates a smaller TreeSummarizedExperiment with only relevant data

**Stage 2: Local filtering on non-sorted columns**
- Once data is in R, apply additional filters on non-sorted columns (e.g., `gene_family_species`)
...
- Adequate disk space if using a persistent DuckDB database file (specify with `dbdir` parameter)
- Sufficient RAM for in-memory operations (default `:memory:`)
- For very large extractions, consider running in a high-memory environment or cloud instance
