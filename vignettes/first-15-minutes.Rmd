...
as_tibble(sampleMetadata[1:5, 1:10])
```
...
You can select samples by phenotype instead of study, and select a few features 
across many samples, or many features across a few samples. If you want many 
features across many samples, you will need to download the data and query 
locally, which is covered in the `Piecewise Workflow` vignette.
...
If `returnSamples` fails:

- `HTTP 429`, slow internet access, or queries that will return a lot of data: 
  download then use local parquet files with `local_files`.
  
- Try inspecting the SQL query that was that your code generated by setting 
  `dry_run = TRUE` and run `dplyr::show_query()`.
