...
```{r discover_refs, eval=interactive()}
...
```{r browse_pathways, eval=interactive()}
...
Now fetch the abundance of a specific butyrate biosynthesis pathway across all samples:

```{r pathway_example, eval=interactive()}
# Use all samples (no filtering by study)
sample_table_all <- sampleMetadata |>
    select(where(~ !all(is.na(.x))))
